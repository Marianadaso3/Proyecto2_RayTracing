...
def vectAdd(V1 = V3(0,0,0), V2 = V3(0,0,0)):
    return V3(V1.x + V2.x, V1.y + V2.y, V1.z + V2.z)

# function to multiply 2 vectors
def vecMultiply(v1, v2):
    return V3(v1[0] * v2[0],v1[1] * v2[1],v1[2] * v2[2])
...
def normalize(vector):
    result = ( abs(vector[0])**2 + abs(vector[1])**2 + abs(vector[2])**2 )**(1/2)
    return V3(vector[0] / result,vector[1] / result,vector[2] / result)

# function to multiply a vector with a number
def multiply(vector, number):
    return V3(vector[0] * number,vector[1] * number,vector[2] * number)
